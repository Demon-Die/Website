  ...
  try {
    ...
    tbody.innerHTML = leaders.map((amb, index) => {
      ...
      if (rank === 1) {
        ...
        badgeHtml = '<span class="px-1.5 py-0.5 text-[9px] bg-yellow-400/20 text-yellow-300 border border-yellow-400/30 rounded font-mono font-bold uppercase">Top 1 Featured</span>';
      } else if (rank === 2) {
        rankStyles = "text-gray-300 font-bold";
        rowStyles = "bg-gray-300/5 hover:bg-gray-300/10 border-l-2 border-gray-300";
        badgeHtml = '<span class="px-1.5 py-0.5 text-[9px] bg-gray-300/20 text-gray-300 border border-gray-300/30 rounded font-mono font-bold uppercase">Top 2 Featured</span>';
      } else if (rank === 3) {
        rankStyles = "text-amber-600 font-bold";
        rowStyles = "bg-amber-600/5 hover:bg-amber-600/10 border-l-2 border-amber-600";
        badgeHtml = '<span class="px-1.5 py-0.5 text-[9px] bg-amber-600/20 text-amber-500 border border-amber-600/30 rounded font-mono font-bold uppercase">Top 3 Featured</span>';
      }
